...
A custom rule implementation for the maven-enforcer plugin that enforces 3rd party dependencies used by a project do not have been released longer than X days/weeks/months/years ago. Certain artifacts/group IDs/version numbers can be excluded using the same configuration that the ![maven-versions-plugin](https://www.mojohaus.org/versions-maven-plugin/rule.html) uses.
